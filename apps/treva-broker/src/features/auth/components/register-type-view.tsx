...
export type CompanySetup = "create" | "join";

export interface RegistrationChoice {
    type: RegistrationType;
    setup: CompanySetup | null;
    /** Trimmed; only set when creating a company — its name is all that step asks for. */
    companyName: string | null;
}

export interface RegisterTypeViewProps {
    /** A choice made earlier, restored when the user comes back with Back. */
    initial?: RegistrationChoice | null;
    /** Called with the completed choice once Continue is pressed. */
    onContinue: (choice: RegistrationChoice) => void;
}
 ...
 * type tiles (step 1); pick Company and, once Continue is pressed, a "Company
 * setup" block unfolds under them with two more rows — that is what moves the
 * meter to step 2, never picking the tile by itself. Choosing "Join Existing
 * Company" grows that row again to hold a company field.
 *
 * The two paths don't share a step count: Individual is type -> credentials
 * (2 steps), Company is type -> setup -> credentials (3) — so the meter's own
 * denominator follows the choice instead of always counting a step Individual
 * never has.
 *
 * Only the card's CONTENT is here. The page, the card itself and the legal line
 * belong to `RegisterShell`, which outlives every step so the card can animate
 * its height as this content grows and shrinks.
 *
 ...
 */
export function RegisterTypeView({ initial, onContinue }: RegisterTypeViewProps) {
    const { locale, t } = useI18n();
    const router = useRouter();
    const copy = t.auth.registerType;

    const [type, setType] = useState<RegistrationType>(initial?.type ?? "individual");
    const [setup, setSetup] = useState<CompanySetup | null>(initial?.setup ?? null);
    const [company, setCompany] = useState("");
    const [companyName, setCompanyName] = useState(initial?.companyName ?? "");
    // Whether the company-setup block has been opened via Continue. Picking
    // the "Company" tile only marks it as the chosen type — it must not, by
    // itself, advance the meter or reveal the setup rows; only pressing
    // Continue does that. Individual never sets this, so it never has a
    // second step here at all.
    // Coming back from the company's credentials step lands on its setup panel.
    const [revealed, setRevealed] = useState(initial?.type === "company");

    const onCompany = type === "company";
    const showSetup = onCompany && revealed;
    // Individual's flow is type -> credentials, two steps total. Company's is
    // type -> company setup -> credentials, three — the meter's denominator
    // changes with the choice rather than counting a step neither path has.
    const totalSteps = onCompany ? 3 : 2;
    const step = showSetup ? 2 : 1;
    // Creating a company needs nothing but its name, so that is all it waits on.
    const canContinue = !showSetup
        ? true
        : setup === "create"
    ...
    function choose(next: RegistrationType) {
        if (next === type) return; // re-clicking the selected tile must not collapse an open setup panel
        setType(next);
        setRevealed(false);
        if (next === "individual") {
            setSetup(null);
    ...
    function goBack() {
        // Back walks out of the company setup panel first, then off the screen.
        if (showSetup) {
            setRevealed(false);
            return;
        }
    ...
    function handleContinue() {
        // Company's first Continue only opens its second step; the type
        // choice itself never does. Every other case hands off to the caller.
        if (onCompany && !revealed) {
            setRevealed(true);
            return;
        }
        onContinue({
            type,
            setup,
            companyName: setup === "create" ? companyName.trim() : null,
        });
    }

    return (
        <>
            {/* 873:60393 — the rule under the header is drawn only once the
                company setup panel is open. */}
            <header
                className={cn("px-8 pt-8 pb-6", showSetup && "border-b border-border-subtle")}
            >
            ...
            <div className="px-8 py-6">
                ...
                {showSetup ? (
                    <div className="pt-4 motion-safe:animate-[step-in_220ms_ease-out]">
